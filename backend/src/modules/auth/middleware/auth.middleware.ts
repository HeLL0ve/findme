...
function extractAccessPayload(token: string): AccessPayload {
  ...
  if (!decoded || typeof decoded !== 'object') {
    throw new AuthError('INVALID_TOKEN', 'Токен невалиден или истёк', 401);
  }
...
export async function authMiddleware(req: Request, _res: Response, next: NextFunction) {
  ...
  try {
    const token = parts[1];
    if (!token) return next(new AuthError('INVALID_AUTH_FORMAT', 'Неверный формат токена', 401));
    ...
    return next(new AuthError('INVALID_TOKEN', 'Токен невалиден или истёк', 401));
  }
}
...
export function adminOnly(req: Request, _res: Response, next: NextFunction) {
  const user = req.user;
  if (!user) return next(new AuthError('NO_AUTH', 'Требуется авторизация', 401));
  if (user.role !== 'ADMIN') return next(new AuthError('FORBIDDEN', 'Доступ только для администраторов', 403));
  return next();
}
