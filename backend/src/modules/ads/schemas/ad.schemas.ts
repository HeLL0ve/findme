...
const photoUrlSchema = z.string().trim().refine(
  ...
  { message: 'Некорректный формат ссылки на фото' },
);
...
export const createAdSchema = z.object({
  ...
  description: z
    .string()
    .trim()
    .min(10, 'Описание слишком короткое (минимум 10 символов)')
    .max(4000, 'Описание слишком длинное'),
  location: z
    .object({
    ...
    .optional(),
  photos: z.array(photoUrlSchema).max(8, 'Можно добавить не более 8 фото').optional(),
});
